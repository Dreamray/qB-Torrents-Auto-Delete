...
(function() {
     ...
     * 只支持qb，在Windows版qb 4.4.3.1、4.5.0 版本中测试通过
    ...
    const
        ...
        timeScale           = 4,   //时间标尺，此时间内平均上传速度低于averageUpSpeedScale会被删，单位分钟。例：值为5表示5分钟前到现在的时间段
    ...
    $.ajax({
        ...
        error: function(){
            console.log('网络错误，请确认qb是否已开启或webui配置');
        }
    });
    ...
    let
        ...
        deleteTorrent = function(hash,name,size,domain,reason,isDeleteFile,callback){
            let
                url = testMode ? '/api/v2/torrents/pause' : '/api/v2/torrents/delete',
                data = testMode ? {hashes: hash} : {hashes: hash, deleteFiles: isDeleteFile}
            ;
            $.ajax({
                method: 'POST',
                url: url,
        ...
        checkAndDelete = function(){
            ...
            $.ajax({
                ...
                success: function(maindata){
                    ...
                    let diskFreeSpace = maindata.server_state.free_space_on_disk,
